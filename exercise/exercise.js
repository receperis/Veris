...
class VocabularyExercise {
  ...
  updateLanguageInfo() {
    ...
    if (info) {
      if (this.selectedLanguage) {
        ...
        const pretty =
          NAME_MAP[this.selectedLanguage] ||
  ...
  loadQuestion() {
    ...
    const progressPercent =
      (this.currentQuestionIndex / this.words.length) * 100;
    ...
    document.querySelector(".source-language").textContent =
      currentWord.sourceLanguage.toUpperCase();
  ...
  async selectAnswer(selectedAnswer, correctAnswer) {
    ...
    this.answeredMap.set(this.currentQuestionIndex, {
  ...
  skipQuestion() {
    ...
    // Show skipped toast
    this.showAnswerNotification(
      `Skipped. Correct: ${this.words[this.currentQuestionIndex].translation}`,
      "skipped"
    );
  ...
  showResults() {
    ...
    document.querySelector(
      ".score-fraction"
    ).textContent = `${this.correctAnswers}/${this.words.length}`;
  ...
  async applyNewTargetLanguage(lang) {
    ...
    clearTimeout(this.pendingAdvance);
    this.showAnswerNotification(
      `Translating to ${this.languageName(lang)}...`,
      "skipped"
    );
    try {
